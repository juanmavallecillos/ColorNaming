...
def ClusterColorNaming(Clusters, options):
    K = options['K']
    threshold = options['single_thr']
    colorSpace = options['colorspace']

    # Constants
    #colors=['Red','Orange','Brown','Yellow','Green','Blue','Purple','Pink','Black','Grey','White']
    colorsSec=['RedBlack', 'GreenBlack', 'YellowBlack', 'BlueBlack', 'BrownBlack', 'PinkBlack', 'PurpleBlack', 'OrangeBlack', 'RedWhite', 'GreenWhite', 'YellowWhite', 'BlueWhite', 'BrownWhite', 'PinkWhite', 'PurpleWhite', 'OrangeWhite', 'RedGrey', 'GreenGrey', 'YellowGrey', 'BlueGrey', 'BrownGrey', 'PinkGrey', 'PurpleGrey', 'OrangeGrey', 'RedBrown', 'RedOrange', 'GreenYellow', 'GreenBlue', 'GreenBrown', 'YellowOrange', 'YellowPink', 'BluePurple', 'BluePink', 'BrownYellow', 'PinkPurple', 'PinkOrange']
	...
	if colorSpace == 'RGB':
            rgbVal = np.reshape(Clusters[i], (1, 1, len(Clusters[i])))
        ...
        for j in range(K):
            index = np.argmax(CDi)
	    if( (j==0) or (CDi[index] > threshold) ):
                labels.append(colors[index])
                weights.append(CDi[index])
                CDi[index] = 0
            else:
                break
        # are the label legal?
        if len(labels) > 1:
            str1 = str(labels[0]) + str(labels[1])
            str2 = str(labels[1]) + str(labels[0])
            if str1 in colorsSec:
                L.append(str1)
            elif str2 in colorsSec:
                L.append(str2)
            else:
                L.append(labels[0])
        else:
